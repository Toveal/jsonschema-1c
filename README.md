...
- **local-date-time** - Формат даты `ЛокальнаяДата (LocalDate)` из 1С. В данной реализации, если строке поставить формат **date-time**, локальная дата не пройдет проверку. Валидными считаются только `ЛокальнаяДатаСоСмещением` и `УниверсальнаяДата`. Данный формат проверяет дату как локальную дату 
